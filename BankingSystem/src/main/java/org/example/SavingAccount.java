...
public class SavingAccount {
    ...
    private static void viewDetails(Connection connection, int id) throws SQLException {
        try {
            statement = connection.prepareStatement("SELECT * FROM saving WHERE id=?");
    ...
    private static void withdraw(Connection connection, int id, int amt) throws SQLException {
        try {
            statement = connection.prepareStatement("SELECT AMT FROM saving WHERE id=?");
            statement.setInt(1, id);
            resultSet = statement.executeQuery();
            ...
            if (currentAmount >= amt) {
                ...
                statement = connection.prepareStatement("UPDATE saving SET AMT=? WHERE id=?");
    ...
    public static void savingRegister(String name,int amount){
        try {
            connection = DriverManager.getConnection(url, username, password);
            statement=connection.prepareStatement("Insert into saving (name,AMT)values(?,?)");
